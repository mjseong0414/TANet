...
def checkpoint_sequence(model_dir, model_name, i):
    ...
    if model_name not in ckpt_dict['latest_ckpt']:
        return None
    latest_ckpt = ckpt_dict['all_ckpts'][model_name][i]
...
def restore(ckpt_path, model):
   ...
   #print("Restoring parameters from {}".format(ckpt_path), file=logf)

def restore_with_log(ckpt_path, model, logf):
    if not Path(ckpt_path).is_file():
        raise ValueError("checkpoint {} not exist.".format(ckpt_path))
    model.load_state_dict(torch.load(ckpt_path))
    print("Restoring parameters from {}".format(ckpt_path))
...
def try_restore_sequence_checkpoints(model_dir, models, i, logf):
    name_to_model = _get_name_to_model_map(models)
    for name, model in name_to_model.items():
        latest_ckpt = checkpoint_sequence(model_dir, name, i)
        if latest_ckpt is not None:
            restore_with_log(latest_ckpt, model, logf)
