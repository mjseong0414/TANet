...
def filter_gt_low_points(gt_boxes,
    ...
    for i, num in enumerate(num_gt_points):
        if num <= point_num_threshold:
            masks = box_np_ops_JRDB.points_in_rbbox(points, gt_boxes[i:i + 1])
...
def remove_points_in_boxes(points, boxes):
    masks = box_np_ops_JRDB.points_in_rbbox(points, boxes)
...
def mask_points_in_corners(points, box_corners):
    surfaces = box_np_ops_JRDB.corner_to_surfaces_3d(box_corners)
...
def noise_per_box(boxes, valid_mask, loc_noises, rot_noises):
    ...
    box_corners = box_np_ops_JRDB.box2d_to_corner_jit(boxes)
...
def noise_per_object_v3_(gt_boxes,
    ...
    origin = [0.5, 0.5, 0] # ################### kitti origin...???
    gt_box_corners = box_np_ops_JRDB.center_to_corner_box3d(
        gt_boxes[:, :3],
        gt_boxes[:, 3:6],
        gt_boxes[:, 6],
        origin=origin,
    ...
    surfaces = box_np_ops_JRDB.corner_to_surfaces_3d_jit(gt_box_corners)
...
def global_rotation(gt_boxes, points, rotation=np.pi / 4):
    ...
    points[:, :3] = box_np_ops_JRDB.rotation_points_single_angle(
        points[:, :3], noise_rotation, axis=2)
    gt_boxes[:, :3] = box_np_ops_JRDB.rotation_points_single_angle(
...
def box_collision_test(boxes, qboxes, clockwise=True):
    ...
    boxes_standup = box_np_ops_JRDB.corner_to_standup_nd_jit(boxes)
    qboxes_standup = box_np_ops_JRDB.corner_to_standup_nd_jit(qboxes)
    for i in range(N):
        for j in range(K):
