...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    ...
    return num_valid_gt, ignored_gt, dc_bboxes
...
def compute_statistics_jit(overlaps,
    ...
    if compute_fp:
        for i in range(det_size):
            if (dt_scores[i] < thresh):
                ignored_threshold[i] = True
    NO_DETECTION = -10000000
    tp, fp, fn, similarity = 0, 0, 0, 0 # true_positive, false_positive, false_negative, similarity
    # thresholds = [0.0]
    # delta = [0.0]
    thresholds = np.zeros((gt_size, ))
    thresh_idx = 0
    ...
    for i in range(gt_size):
        ...
        for j in range(det_size):
            ...
            if (ignored_threshold[j]):
                continue
            overlap = overlaps[j, i]
            # print("3333333333333333333333333333333333333333333333333 >> ", overlap)
            # print("4444444444444444444444444444444444444444444444444 >> ", min_overlap)
            # print("555555555555555555555555555555555555555555555555 >> ", max_overlap)
            dt_score = dt_scores[j]
            if (not compute_fp and (overlap > min_overlap)
                    and dt_score > valid_detection):
                det_idx = j
    ...
    if compute_fp:
        for i in range(det_size):
            if (not (assigned_detection[i] or ignored_threshold[i])):
                fp += 1
        nstuff = 0
        # if metric == 0:
        #     overlaps_dt_dc = image_box_overlap(dt_bboxes, dc_bboxes, 0)
...
def fused_compute_statistics(overlaps,
    ...
    for i in range(gt_nums.shape[0]):
        for t, thresh in enumerate(thresholds):
            # print("11111111111111111111111111111111111111111111111111111111>>> ", overlaps)
            overlap = overlaps[dt_num:dt_num + dt_nums[i], gt_num:
                               gt_num + gt_nums[i]]
            # print("222222222222222222222222222222222222222222222222222222@>>>>>>>>>> " ,overlap)
...
def calculate_iou_partly(gt_annos, dt_annos, metric, num_parts=50):
    ...
    total_dt_num = np.stack([len(a["name"]) for a in dt_annos], 0) # 22 values
    total_gt_num = np.stack([len(a["name"]) for a in gt_annos], 0) # 300 values
    num_examples = len(gt_annos) # 10 values
    num_parts = 1 #######################################################################################################
    split_parts = get_split_parts(num_examples, num_parts)
    parted_overlaps = []
    example_idx = 0
    for num_part in split_parts:
        gt_annos_part = gt_annos[example_idx:example_idx + num_part] # gt_annos[0:10]
        dt_annos_part = dt_annos[example_idx:example_idx + num_part] # dt_annos[0:10]
        if metric == 0:
            ...
            gt_boxes = np.concatenate(
                [loc, dims, rots[..., np.newaxis]], axis=1) # array([[loc[0], loc[1], dims[0], dims[1], rots]])
            loc = np.concatenate(
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty):
    ...
    for i in range(len(gt_annos)):
        ...
        num_valid_gt, ignored_gt, dc_bboxes = rets
    ...
    return (gt_datas_list, dt_datas_list, ignored_gts, dontcares,
            total_dc_num, total_num_valid_gt)
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                for j, num_part in enumerate(split_parts):
                    ...
                    fused_compute_statistics(
                        ...
                        min_overlap=min_overlap,
                        thresholds=thresholds,
                        compute_aos=compute_aos) # create pr values
                    idx += num_part
                for i in range(len(thresholds)):
...
def do_eval_v2(gt_annos,
    ...
    # ret = eval_class_v3(gt_annos, dt_annos, current_classes, difficultys, 0, min_overlaps, compute_aos)
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, difficultys=[0, 1, 2], return_data=False):
    ...
    if not isinstance(current_classes, (list, tuple)):
        current_classes = [current_classes] # ['Pedestrian']
    current_classes_int = [] 
    ...
    current_classes = current_classes_int # 1
    min_overlaps = min_overlaps[:, :, current_classes] # min_overlaps has 1 column values 0.5, 0.5, 0.5, 0.5, 0.25, 0.25
    result = ''
    # check whether alpha is valid
    compute_aos = False
    # for anno in dt_annos:
