...
def get_label_path(idx, prefix, training=True, relative_path=True, exist_check=True):
    return get_kitti_info_path(idx, prefix, target_label_name, '.txt', training,
                               relative_path, exist_check)

def get_velodyne_path(idx, prefix, training=True, relative_path=True, exist_check=True):
    return get_kitti_info_path(idx, prefix, 'velodyne', '.npy', training,
                               relative_path, exist_check)
...
def get_kitti_image_info(path,
    ...
    def map_func(idx):
        ...
        if with_imageshape:
            ...
            if relative_path:
                #print("relative_path yes")
                #print(str(root_path) + "_root path")
                #print(str(img_path) + "_img path")
...
#             if relative_path:
#                 label_path = str(root_path / label_path)
#             annotations = get_label_anno(label_path) # get_label_anno함수에서 text 파일 각 column의 이름들이 정해짐 -> JRDB에 맞게 수정하기
#         if annotations is not None:
#             image_info['annos'] = annotations
#             add_difficulty_to_annos(image_info) # occlusion과 truncation에 따라서 difficulty가 결정됨
#         return image_info

#     for idx in image_ids:
#         image_infos = map_func(idx) # map_func함수 실행
...
def get_label_anno(label_path):
    ...
    for k in range(len(lines)-1):
        if int(lines[k].split(" ")[15]) >= 10: # first remove lines in this condition
