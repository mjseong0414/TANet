...
#             annotations = get_label_anno(label_path, num_points_info)
#         if calib:
...
def get_class_to_label_map_JRDB():
    class_to_label = {
        'Pedestrian': 0,
        'DontCare': -1,
    }
...
def get_classes(target_data):
    if target_data == 'JRDB':
        return get_class_to_label_map_JRDB().keys()
    else:
...
def get_label_anno(label_path, num_points_info):
    annotations = {}
    annotations.update({
        'name': [],
        'truncated': [],
    ...
    # dimensions is lidar coordinates
    ...
    # Add 'num_points_in_gt'
    if num_points_info:
        annotations['num_points_in_gt'] = np.array([int(x[16]) for x in content])
    return annotations
