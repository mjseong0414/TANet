...
from second.core import preprocess_JRDB as prep
...
from second.data import kitti_common_JRDB as kitti
...
def prep_pointcloud(input_dict,
    ...
    if training:
        ...
        if group_ids is not None:
            group_ids = group_ids[selected]
        gt_boxes = box_np_ops_JRDB.box_camera_to_lidar_rough(gt_boxes)
        if remove_unknown:
    ...
    if anchor_area_threshold >= 0:
        coors = coordinates
        dense_voxel_map = box_np_ops_JRDB.sparse_sum_for_anchors_mask(
            coors, tuple(grid_size[::-1][1:]))
        dense_voxel_map = dense_voxel_map.cumsum(0)
        dense_voxel_map = dense_voxel_map.cumsum(1)
        anchors_area = box_np_ops_JRDB.fused_get_anchors_area(
...
def _read_and_prep_v9(info, root_path, num_point_features, prep_func, bev_target=None):
    ...
    if 'annos' in info:
        ...
        # gt_boxes = box_np_ops_JRDB.camera_to_lidar_rough(gt_boxes, dims[:,1])
