...
from second.pytorch.core import box_torch_ops_JRDB
...
class RPN(nn.Module):
    def __init__(self,
                 ...
                 layer_nums=[3, 5, 5],
                 layer_strides=[2, 2, 2],
                 num_filters=[64, 64, 128], # [128, 128,256]
                 upsample_strides=[1, 2, 4],
                 num_upsample_filters=[128, 128, 128], #[256, 256, 256]
                 num_input_filters=64, # 64
        ...
        self.block1 = Sequential(
            nn.ZeroPad2d(1),
            Conv2d(
                #num_input_filters, num_filters[0], 3, stride=layer_strides[1]),
    ...
    def forward(self, x, bev=None):
        #print("#################### x", x.shape) # [2, 64, 417, 417]
        x = self.block1(x)
        #print("#################### x1", x.shape) # [2, 64, 417, 417]
        up1 = self.deconv1(x)
        #print("#################### up1", up1.shape) # [2, 128, 417, 417]
        if self._use_bev:
            bev[:, -1] = torch.clamp(
                torch.log(1 + bev[:, -1]) / np.log(16.0), max=1.0)
            x = torch.cat([x, self.bev_extractor(bev)], dim=1)
        x = self.block2(x)
        #print("#################### x2", x.shape) # [2, 128, 209, 209]
        up2 = self.deconv2(x)
        #print("#################### up2", up2.shape) # [2, 128, 418, 418]
        x = self.block3(x)
        #print("#################### x3", x.shape) # [2, 256, 105, 105]
        up3 = self.deconv3(x)
        #print("#################### up3", up3.shape) # [2, 128, 420, 420]
...
class VoxelNet(nn.Module):
    ...
    def forward(self, example,  refine_weight=2):
        ...
        num_points = example["num_points"] # number of points in each voxels
        coors = example["coordinates"] # voxel coordinates??
    ...
    def compute_predict(self, batch_box_preds, batch_cls_preds,
                        batch_dir_preds, batch_imgidx, batch_anchors_mask, num_class_with_bg):
        predictions_dicts = []
        for box_preds, cls_preds, dir_preds, img_idx, a_mask in zip(
            ...
            if self._use_rotate_nms:
                nms_func = box_torch_ops_JRDB.rotate_nms
            else:
                nms_func = box_torch_ops_JRDB.nms
            selected_boxes = None
            ...
            if self._multiclass_nms:
                ...
                selected_per_class = box_torch_ops_JRDB.multiclass_nms(
                    nms_func=nms_func,
            ...
            if selected_boxes is not None:
                ...
                final_box_preds_camera = box_torch_ops_JRDB.box_lidar_to_camera_rough(
                    final_box_preds)
                locs = final_box_preds_camera[:, :3] # x, y, z value of predicted bbox in camera coordinates
                dims = final_box_preds_camera[:, 3:6]  # w, h, l value of predicted bbox in camera coordinates
                angles = final_box_preds_camera[:, 6] #  # angle value of predicted bbox in camera coordinates
                camera_box_origin = [0.5, 1.0, 0.5]
                box_corners = box_torch_ops_JRDB.center_to_corner_box3d(
                    locs, dims, angles, camera_box_origin, axis=1)
                """
                <bbox>
                box_corners_in_image = box_torch_ops_JRDB.project_to_image(
                    box_corners, P2)
    ...
    def avg_forward_time(self):
        # return self._total_forward_time / self._total_inference_count, self._total_forward_time, self._total_inference_count
