...
class PACALayer(nn.Module):
    ...
    def forward(self, x):
        # x has nan
        # x_nan = torch.isnan(x)
        # x_numpy = x_nan.cpu().numpy()
        # for i in range(len(x_numpy)):
        #     for m in range(len(x_numpy[i])):
        #         for e in range(len(x_numpy[i][m])):
        #             if int(x_numpy[i][m][e]) == 1:
        #                 print(x_numpy[i][m], i, m)
...
class PillarFeature_TANet(nn.Module):
    ...
    def forward(self, features, num_voxels, coors):
        ...
        features = self.VoxelFeature_TA(points_mean, features) #################################################### NaN
