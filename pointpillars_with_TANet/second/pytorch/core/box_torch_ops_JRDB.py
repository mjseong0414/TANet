...
################################################################################
def lidar_to_camera_rough(points,h):
    x_camera = -points[:,1].reshape(-1,1)
    y_camera = -points[:,2].reshape(-1,1) + 0.5*h
    z_camera = points[:,0].reshape(-1,1)
    # camera_points = np.concatenate([x_camera, y_camera, z_camera], axis=1)
    camera_points = torch.cat([x_camera, y_camera, z_camera], dim=-1)
...
################################################################################
def box_lidar_to_camera_rough(data):
    xyz_lidar = data[..., 0:3]
    w, l, h = data[..., 3:4], data[..., 4:5], data[..., 5:6]
    r = data[:, 6:7]
    xyz = lidar_to_camera_rough(xyz_lidar,h)
    return torch.cat([xyz, l, h, w, r], dim=-1)
################################################################################
