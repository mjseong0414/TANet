...
import torchplus
import second.data.kitti_common_JRDB as kitti
from second.builder import target_assigner_builder_JRDB, voxel_builder_JRDB
from second.data.preprocess_JRDB import merge_second_batch
from second.protos import pipeline_pb2
from second.pytorch.builder import (box_coder_builder_JRDB, input_reader_builder_JRDB,
                                      lr_scheduler_builder_JRDB, optimizer_builder_JRDB,
                                      second_builder_JRDB)
from second.utils.eval_JRDBV2 import get_coco_eval_result, get_official_eval_result
...
def example_convert_to_torch(example, dtype=torch.float32,
    ...
    example_torch = {}
    float_names = [
        "voxels", "anchors", "reg_targets", "reg_weights", "bev_map", "gt_boxes"
    ]
...
def train(config_path,
    ...
    optimizer = optimizer_builder_JRDB.build(optimizer_cfg, net.parameters())
    if train_cfg.enable_mixed_precision:
    ...
    torchplus.train.try_restore_latest_checkpoints(model_dir,
                                                   [mixed_optimizer])
    lr_scheduler = lr_scheduler_builder_JRDB.build(optimizer_cfg, optimizer, gstep)
    if train_cfg.enable_mixed_precision:
    ...
    # PREPARE INPUT
    ######################
    dataset = input_reader_builder_JRDB.build(
        input_cfg,
        model_cfg,
        training=True,
    ...
    clear_metrics_every_epoch = train_cfg.clear_metrics_every_epoch # false
...
#                    ckpt_start_time = time.time()
            ...
            if  model_cfg.rpn.module_class_name == "PSA" or model_cfg.rpn.module_class_name == "RefineDet":
                ...
                print('Before Refine:')
                result, mAPbev, mAP3d = get_official_eval_result(gt_annos, dt_annos_coarse, class_names,
                                                                                  return_data=True)
                ...
                print('After Refine:')
                result, mAPbev, mAP3d = get_official_eval_result(gt_annos, dt_annos_refine, class_names,
                                                                                  return_data=True)
                dt_annos = dt_annos_refine
            else:
                result, mAPbev, mAP3d = get_official_eval_result(gt_annos, dt_annos, class_names,
                                                                              return_data=True)
            print(result, file=logf)
            print(result)
            writer.add_text('eval_result', result, global_step)
...
def comput_kitti_output(predictions_dicts,batch_image_shape,
    ...
    for i, preds_dict in enumerate(predictions_dicts):
        ...
        if preds_dict["box3d_camera"] is not None:
            ...
            num_example = 0
            """
            box : 카메라 좌표계 상 3D box값
            box_lidar : lidar 좌표계 상 3D box값
            """
...
def predict_kitti_to_anno(net,
    ...
    """
    평균 fps : (frame 수) / (frame당 detection에 걸린 시간의 총 합)
    """
    if use_coarse_to_fine:
        ...
        # torch.cuda.synchronize()
        # inf_time = time.time()-inference_time1
        # t = time.time()
...
def evaluate(config_path,
    ...
    if result_path is None:
        result_path = model_dir / result_name # PosixPath('train_16_ped_cycle_tanet/eval_results')
    else:
    ...
    if  model_cfg.rpn.module_class_name == "PSA" or model_cfg.rpn.module_class_name == "RefineDet":
        ...
        """
        eval_Dataloader에서 데이터 하나씩 뽑아서 model에 inference 돌리는 부분인듯?
        predict_kitti_to_anno함수의 predictions_dicts_coarse, predictions_dicts_refine = net(example)부분이 example하나 당 network를 통과시켜서 detection을 마무리하는 부분이니까 여기서 inference time 측정하면 될듯
        """
        # pure_inference_time = 0 # inference time 총합에 사용할 변수
        for example in iter(eval_dataloader):
            example = example_convert_to_torch(example, float_dtype)
            if pickle_result:
                # coarse, refine, inf_time = predict_kitti_to_anno(
                #     net, example, class_names, center_limit_range,
                #     model_cfg.lidar_input,use_coarse_to_fine = True, global_set =None)
                coarse, refine = predict_kitti_to_anno(
                    net, example, class_names, center_limit_range,
                    model_cfg.lidar_input,use_coarse_to_fine = True, global_set =None)
                dt_annos_coarse += coarse # kitti label 형식으로 만들어진 coarse와 
                dt_annos_refine += refine
                # pure_inference_time += inf_time
            else:
    ...
    # avg_forward_time, total_forward_time, total_inf_count = net.avg_forward_time
    print(f"avg forward time per example: {net.avg_forward_time:.3f}")
    ...
    if not predict_test:
        ...
        # result = get_coco_eval_result(gt_annos, dt_annos, class_names) # only use data_annotation_refine when we implement coco eval
        # print(result)
        if pickle_result:
            #refine pickle dump
            if model_cfg.rpn.module_class_name == "PSA" or model_cfg.rpn.module_class_name == "RefineDet":
                ...
                with open(result_path_step / "result_coarse_trainset_005.pkl", 'wb') as f:
                    pickle.dump(dt_annos, f)
               
def evaluateV2(config_path,
             model_dir,
             result_path=None,
             predict_test=False,
             ckpt_path=None,
    ...
    for i in range(86):
        ...
        logf = open(log_path, 'a')
        if ckpt_path is None:
            torchplus.train.try_restore_sequence_checkpoints(model_dir, [net], i, logf) 
            # log를 결과로 저장하고 싶지 않으면 checkpoint.py의 try_restore_sequence_checkpoints와 restore함수에서 logf를 다 지우자
            # torchplus.train.try_restore_latest_checkpoints(model_dir, [net]) # original code
        else:
            torchplus.train.restore(ckpt_path, net)
        ...
        if not predict_test:
            ...
            if model_cfg.rpn.module_class_name == "PSA" or model_cfg.rpn.module_class_name == "RefineDet":
                print('Before Refine:')
                result_coarse = get_official_eval_result(gt_annos, dt_annos_coarse, class_names)
                print(result_coarse, file=logf)
                print(result_coarse)

                print('After Refine:')
                result_refine = get_official_eval_result(gt_annos, dt_annos_refine, class_names)
                print(result_refine, file=logf)
                print(result_refine)
                # result = get_coco_eval_result(gt_annos, dt_annos_refine, class_names)
                dt_annos = dt_annos_refine
                # print(result)
