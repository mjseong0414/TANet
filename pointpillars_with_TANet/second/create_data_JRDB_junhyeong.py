...
from second.data import kitti_common_junhyeong as kitti
...
def create_kitti_info_file(target_data,
                           data_path,
                           save_path=None,
                           create_trainval=False,
                           relative_path=True):
    
    if target_data =='KITTI':
        ...
        val_img_ids = _read_imageset_file("./data/ImageSets/val.txt")
        trainval_img_ids = _read_imageset_file("./data/ImageSets/trainval.txt")
        #test_img_ids = _read_imageset_file("./data_JRDB/ImageSets/test.txt")
    ...
    if target_data == 'KITTI':
        is_exist_calib = True
        num_points_info = False
    else:
        is_exist_calib = False
        num_points_info = True
    #############################################################################
    # Training set info pickle file
    ...
    #############################################################################
    # Validation set info pickle file
...
def create_groundtruth_database(data_path,
    ...
    if used_classes is None:
        used_classes = list(kitti.get_classes(target_data))
    ...
    for info in prog_bar(kitti_infos):
        ...
        if target_data == 'KITTI':
            ...
            rbbox_lidar = box_np_ops.box_camera_to_lidar_rough(rbbox_cam)
        ###########################################################################
